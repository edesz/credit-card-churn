...
So, on its own, the churn rate of 16% at client's bank is not a concern. However, since the client has observed churn to be increasing, this could be a concern in the near future.

Using the assumed estimated revenue and costs, which will be defined in [the next step](./03_costs.md), the impact of credit card customer churn to the client is a loss of approximately 508 dollars of CLV (CLV) per customer, or 10% of overall CLV, using the client's provided random sample of data. If the cost to acquire a new customer is taken into account (CaC), which we assume to be $200 per customer ([1](https://www.reviewtrackers.com/blog/bank-customer-retention), [2](https://firstpagesage.com/seo-blog/average-customer-acquisition-cost-cac-in-banking/)), then this adds up to a loss of 708 dollars per customer, or a total of 1,151,828 dollars due to churn. If customer churn continues at the same rate, then this cost will be incurred each year. If churn is growing, as has been observed, then this annual cost will also grow.

There are two metrics for which the bank is in danger of exceeding industry standards

1. As mentioned above, the current churn rate at the Bank is ~16%, which is just below the threshold for the financial sector of ~19%.
2. In the U.S., the average CLV per credit card customer across all financial services is approximately 808 dollars. At the client's bank, the CLV of remaining (non-churned) customers is 877 dollars, which is just above this threshold.

The client's observation that more and more customers are churning suggests churn rate is growing which would cause both metrics to exceed industry standards. When this happens, the bank loses revenue and incurs increased CaC relative to its competitors.
